...
namespace BlazorStore.Controllers
{
    ...
    public class PaymentIntentApiController : Controller
    {
        ...
        public async Task<IActionResult> ProcessAsync()
        {
            ...
            await _emailSender.SendEmailAsync(user.Email, "R-KOM ir saņēmis jūsu pasūtījumu",
                $"<h1>Jūs veiksmīgi esat veicis pasūtījumu R-KOM veikalā plkst. {(currentTime).ToString("dd MMMM yyyy - hh:mm tt")}</h1> <br> <a style='background-color: #555555;border: none;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;' href='{HtmlEncoder.Default.Encode(callbackUrl)}'>Clicking here to izsekojiet sūtījumu</a>  <br> <h3>Your Tracking Number is: {TrackingNumber} </h3> <br>");


            await _emailSender.SendEmailAsync("ralfs.m.abols@gmail.com", "Jauns pasūtījums R-KOM",
               $"<h1>Esat saņēmis jaunu pasūtījumu {(currentTime).ToString("dd MMMM yyyy - hh:mm tt")}</h1> <br> <a style='background-color: #555555;border: none;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;' href='{HtmlEncoder.Default.Encode(callbackUrl)}'>Clicking here to go to order</a>  <br> <h3>Tracking Number is: {TrackingNumber} </h3> <br>");
